# Window Quilt 1.6 — active layouts
...
- **Close set…** closes only the listed members. Quilt uses their native close buttons, never quits the app or discards save prompts. Closing is not part of Quilt's arrangement Undo history. A refusal or dialog stops the batch; already completed closes are not rolled back.
- Active edits affect the running set. They do not overwrite a saved setup template.
- Newly opened layouts retain exact window IDs and the app's process-launch identity across Quilt restarts. Membership reconciles as windows close; a restarted app cannot inherit stale destructive targets.
...
- Restarted Quilt and verified tracked identities/counts survived. Restarting the fixture removed old process membership; its new windows were detected afresh.
...
`Tests/ActiveLayoutFixture/main.swift` is a disposable native fixture, with an optional `/private/tmp/quilt-active-fixture/block-close` marker to present a confirmation instead of closing. It writes its window IDs, frames and Spaces to `/private/tmp/quilt-active-fixture/windows.json`. After recording the named before/after reports for the scenarios above, run:

```sh
python3 Tests/ActiveLayoutFixture/verify.py /private/tmp/quilt-active-fixture
```
